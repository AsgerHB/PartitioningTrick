...
%A switch in the circuit must be toggled by a controller to regulate the output voltage with the aim of keeping it close to the target (15V).
...
%The latter models a ball that preserves a varying amount of energy when it impacts with the ground.
...
%Both systems are expressed as IMDPs in the specification language of \uppaalstratego.
Using \uppaalstratego, models are created of IMDPs that correspond to the case studies,
with a partition $\partition_i$ such that there are $i$ regions along each axis. % TODO: Update the experiment so this is no longer the case
