...
% Is integrated the right word to use here? It is the sum each time-step.
...
%A strategy is learned using the region indices as reward signals.
...
For the boost converter, the voltage furthest from the 15V target is picked. 
The remaining state vareiables are the current and the consumption.
...
The $\alphamean$, is set to pick the centre of the region, while $\alphaimax$ is picked analogously to $\alphaimin$.
...
\begin{figure}[t]
  ...
  \end{minipage}
  \caption{Learning outcomes of the bouncing ball (left) and the DC-DC boost converter (right)}
  \label{fig:casestudies}
\end{figure}

Figure~\ref{fig:casestudies} shows the expected reward of an agent trained for a constant number of episodes, 
on IMDPs of varying granularity $i$.
It can be seen that for both agents, the adversary modelled as $\alphaimin$ generally acts as 
