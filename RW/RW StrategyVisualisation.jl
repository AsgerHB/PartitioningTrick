...
get_action_names(selected_file)
...
function get_action(regressors, state)
	actions = [k for (k, _) in regressors]

	lowest_outcome = Inf
	cheapest_action = nothing
	for action in actions
		outcome = get_predicted_outcome(regressors[action], state)
...
function get_policy(file)
	...
	policy = (state) -> get_action(regressors, state)
end

# ╔═╡ 79fb04f7-2d45-454f-89c5-7312d4aeadde
imported_policy = get_policy(selected_file)
...
test_policy = get_policy(selected_file)
...
`selected_discrete_file` = $(@bind selected_discrete_file PlutoUI.FilePicker([MIME("application/json")]))
"""

# ╔═╡ d828cba1-a6bd-43f5-a87c-74f545f8a0ec
if selected_discrete_file == nothing
	md"""
!!! danger "Error"
	# Please select file
"""
...
md"""
## Methods to read the discrete policy file
"""
...
function get_actions_discrete(jsondict2, state)
	actions = Dict(action => NaN for (action, _) in jsondict2["actions"])
	best = Inf
	for (action, _) in actions
		actions[action] = get_predicted_outcome_discrete(jsondict2["regressors"], state, action)
		best = min(best, actions[action])
	end
	Set(action 
		for (action, expectation) in actions
		if expectation == best)
end

# ╔═╡ dec3cbbd-a271-4f60-a5f4-c44069e54f53
# Nondeterministic
function get_discrete_policy(file)
	jsondict = file["data"] |> IOBuffer |> JSON.parse
...
discrete_policy = get_discrete_policy(selected_discrete_file)

# ╔═╡ e63bc1c4-e746-4d36-8d76-2890d5019067
md"""
## Drawing the discrete policy
Draw a 2D policy
"""

# ╔═╡ 9825a0aa-1bde-4061-a4b3-0ac19f581bfd
function draw_discrete(policy::Function, x_min, x_max, y_min, y_max; colors=[:blue, :yellow], actions=["action1", "action2"], plotargs...)
...
@bind G NumberField(0:0.01:1, default=0.01)

# ╔═╡ 398dec96-6309-40c1-af21-ee6c0a2c7ddf
index_max = ceil(1/G)

# ╔═╡ c1290134-68e1-43a2-88f2-4d20d2040c7d
draw_discrete(discrete_policy, 0, index_max, 0, index_max,
	colors=[slow_color, fast_color, either_color],
	actions=["slow", "fast", "either"],
	xlabel="ix",
	ylabel="it",
	legend=:topleft)
